...
const BookCard = ({ book, isPurchased = false }) => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:4000/api'}/marketplace/cart/items`, {
        method: 'POST',
        headers: {
    ...
    <>
      <div className="bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden flex flex-col">
        <div className="relative">
          <img 
            src={book.coverImage ? (book.coverImage.startsWith('/') ? `${(import.meta.env.VITE_API_BASE_URL || 'http://localhost:4000/api').replace('/api', '')}${book.coverImage}` : book.coverImage) : 'https://via.placeholder.com/300x450.png?text=Book+Cover'}
            alt={book.title}
