...
const ManageCoursesPage = () => {
  ...
  useEffect(() => {
    ...
    const loadCourses = () => load(user._id);

    loadCourses(); // Initial load

    // Listen for changes in localStorage to refetch courses
    const handleStorageChange = (event) => {
      if (event.key === 'coursesUpdated') {
        loadCourses();
      }
    };
          ...
          <div className="relative z-10 w-full max-w-2xl bg-white rounded-xl shadow-2xl p-6">
            ...
            <div className="mt-4 grid md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-medium">{selectedCourse.instructorName || 'Unknown'}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">
                  {selectedCourse.createdAt 
                    ? new Date(selectedCourse.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) 
                    : '—'}
                </p>
              </div>
