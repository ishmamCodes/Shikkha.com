...
const LoginSignup = () => {
  ...
  const API_BASE_URL = "https://shikkha-com.onrender.com/api/auth";
  ...
  const validateForm = () => {
    const errors = {};
    if (isSignup && !formData.fullName) errors.fullName = "Full name is required";
    if (!formData.email) errors.email = "Email is required";
    if (!formData.password) errors.password = "Password is required";
    if (isSignup && !formData.dateOfBirth) errors.dateOfBirth = "Date of birth is required";
    ...
    try {
      ...
      const normalizedUser = {
        ...
        avatarUrl: user?.avatarUrl
          ? (user.avatarUrl.startsWith('http') ? user.avatarUrl : `${SERVER_ORIGIN}${user.avatarUrl}`)
          : ''
      };

      localStorage.setItem("token", token);
      localStorage.setItem("user", JSON.stringify(normalizedUser));
      updateUser(normalizedUser); // Update the context

      // Ensure admin-specific storage for ProtectedAdminRoute
      if (normalizedUser.role === 'admin') {
        localStorage.setItem('adminToken', token);
        localStorage.setItem('adminUser', JSON.stringify(normalizedUser));
      }

      // After login, always show homepage first. Dashboard can be accessed via navbar.
      navigate('/');
    } catch (err) {
