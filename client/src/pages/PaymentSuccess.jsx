      ...
      const handlePaymentSuccess = async () => {
        ...
        try {
          ...
          if (token) {
            ...
            const webhookResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:4000/api'}/payments/trigger-webhook`, {
              method: 'POST',
              headers: { 
            ...
            if (paymentType === 'book' || paymentType === 'cart') {
              ...
              const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:4000/api'}/students/purchases`, {
              ...
              const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:4000/api'}/students/enrolled-courses`, {
                headers: { 'Authorization': `Bearer ${token}` }
              });
              if (response.ok) {
                const data = await response.json();
