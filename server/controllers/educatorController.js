...
// Admin: list educators with basic fields
export const listEducators = async (req, res) => {
  try {
    ...
    console.error('[listEducators]', err);
    return res.status(500).json({ success: false, message: 'Failed to load educators' });
  }
};

// Admin: suspend educator
export const suspendEducator = async (req, res) => {
  try {
    if (req.user?.role !== 'admin') return res.status(403).json({ success: false, message: 'Admin only' });
    const { id } = req.params;
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(400).json({ success: false, message: 'Invalid id' });
    const updated = await Educator.findByIdAndUpdate(id, { status: 'suspended' }, { new: true }).select('_id fullName email status');
    if (!updated) return res.status(404).json({ success: false, message: 'Educator not found' });
    return res.status(200).json({ success: true, data: updated });
  } catch (err) {
    console.error('[suspendEducator]', err);
    return res.status(500).json({ success: false, message: 'Failed to suspend educator' });
  }
};

// Admin: activate educator
export const activateEducator = async (req, res) => {
  try {
    if (req.user?.role !== 'admin') return res.status(403).json({ success: false, message: 'Admin only' });
    const { id } = req.params;
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(400).json({ success: false, message: 'Invalid id' });
    const updated = await Educator.findByIdAndUpdate(id, { status: 'active' }, { new: true }).select('_id fullName email status');
    if (!updated) return res.status(404).json({ success: false, message: 'Educator not found' });
    return res.status(200).json({ success: true, data: updated });
  } catch (err) {
    console.error('[activateEducator]', err);
    return res.status(500).json({ success: false, message: 'Failed to activate educator' });
  }
};

// Admin: delete educator (and detach from courses)
export const deleteEducator = async (req, res) => {
  try {
    if (req.user?.role !== 'admin') return res.status(403).json({ success: false, message: 'Admin only' });
    const { id } = req.params;
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(400).json({ success: false, message: 'Invalid id' });
    const edu = await Educator.findById(id);
    if (!edu) return res.status(404).json({ success: false, message: 'Educator not found' });
    // Remove educator reference from their courses
    await Course.updateMany({ instructor: id }, { $unset: { instructor: 1, instructorName: 1 } });
    await Educator.findByIdAndDelete(id);
    return res.status(200).json({ success: true, message: 'Educator deleted' });
  } catch (err) {
    console.error('[deleteEducator]', err);
